...
func (c *SAPAPICaller) AsyncPostPurchaseRequisition(
...
func (c *SAPAPICaller) HeaderItem(headerItem *requests.HeaderItem) {
	err := c.callPurchaseRequisitionSrvAPIRequirementHeaderItem("A_PurchaseRequisition", headerItem)
	if err != nil {
		c.log.Error(err)
		return
	}
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": headerItem, "function": "PurchaseRequisitionHeaderItem"})
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) callPurchaseRequisitionSrvAPIRequirementHeaderItem(api string, headerItem *requests.HeaderItem) error {
	body, err := json.Marshal(headerItem)
	if err != nil {
		return xerrors.Errorf("API request error: %w", err)
	}
	url := strings.Join([]string{c.baseURL, "API_PURCHASE_REQUISITION_SRV", api}, "/")
	...
	headerItem.PurchaseRequisition = resBody.D.PurchaseRequisition
	return nil
}
